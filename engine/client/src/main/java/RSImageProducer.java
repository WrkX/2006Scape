...
final class RSImageProducer implements ImageProducer, ImageObserver {
	...
	public void drawGraphics(int i, Graphics g, int k, float opacity) {
		method239();
		Graphics copy = g.create();
		if (copy instanceof Graphics2D) {
			((Graphics2D) copy).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, opacity));
		}
		copy.drawImage(anImage320, k, i, this);
		copy.dispose();
	}
