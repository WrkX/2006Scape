...
public final class Background extends DrawingArea {
	...
	/**
	 * Draw this background at a different width while keeping the left and
	 * right edges at their native size. Only the middle section is sampled to
	 * fill the extra space, so borders stay crisp and the image is never scaled
	 * vertically.
	 */
	public void drawHorizontalStretched(int x, int y, int targetWidth, int edgeWidth) {
		drawStretched(x, y, targetWidth, anInt1453, edgeWidth, 0);
	}

	/**
	 * Draw this background at a different size while preserving fixed-size
	 * borders. The center is nearest-neighbour sampled, which suits the
	 * client's pixel-art surfaces and avoids filtering the texture.
	 */
	public void drawStretched(int x, int y, int targetWidth, int targetHeight, int edgeWidth, int edgeHeight) {
		int sourceWidth = anInt1452;
		int sourceHeight = anInt1453;
		...
		int horizontalEdge = Math.min(edgeWidth, Math.min(sourceWidth / 2, targetWidth / 2));
		int verticalEdge = Math.min(edgeHeight, Math.min(sourceHeight / 2, targetHeight / 2));
		int sourceMiddleWidth = sourceWidth - horizontalEdge * 2;
		int targetMiddleWidth = targetWidth - horizontalEdge * 2;
		int sourceMiddleHeight = sourceHeight - verticalEdge * 2;
		int targetMiddleHeight = targetHeight - verticalEdge * 2;

		for (int targetY = 0; targetY < targetHeight; targetY++) {
			int drawY = y + anInt1455 + targetY;
			if (drawY < DrawingArea.topY || drawY >= DrawingArea.bottomY) {
				continue;
			}

			int sourceY;
			if (targetY < verticalEdge) {
				sourceY = targetY;
			} else if (targetY >= targetHeight - verticalEdge) {
				sourceY = sourceHeight - (targetHeight - targetY);
			} else if (sourceMiddleHeight > 0 && targetMiddleHeight > 0) {
				sourceY = verticalEdge + (targetY - verticalEdge) * sourceMiddleHeight / targetMiddleHeight;
			} else {
				sourceY = Math.min(targetY, sourceHeight - 1);
			}

			int sourceRow = sourceY * sourceWidth;
			int targetRow = drawY * DrawingArea.width;
			for (int targetX = 0; targetX < targetWidth; targetX++) {
				int drawX = x + anInt1454 + targetX;
				if (drawX < DrawingArea.topX || drawX >= DrawingArea.bottomX) {
					continue;
				}

				int sourceX;
				if (targetX < horizontalEdge) {
					sourceX = targetX;
				} else if (targetX >= targetWidth - horizontalEdge) {
					sourceX = sourceWidth - (targetWidth - targetX);
				} else if (sourceMiddleWidth > 0 && targetMiddleWidth > 0) {
					sourceX = horizontalEdge + (targetX - horizontalEdge) * sourceMiddleWidth / targetMiddleWidth;
				} else {
					sourceX = Math.min(targetX, sourceWidth - 1);
				}

				byte paletteIndex = aByteArray1450[sourceRow + sourceX];
				if (paletteIndex != 0) {
					DrawingArea.pixels[targetRow + drawX] = anIntArray1451[paletteIndex & 0xff];
				}
			}
		}
	}
