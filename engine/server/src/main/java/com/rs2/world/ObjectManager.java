...
public class ObjectManager {
	...
	public void process() {
		for (final Object o : new ArrayList<Object>(objects)) {
			if (o.tick > 0) {
				o.tick--;
			} else {
				if (WorldObjectService.getInstance().applyTimedExpire(this, o)
						== WorldObjectService.MutationResult.APPLIED) {
					toRemove.add(o);
				}
			}
		}
	...
	public void removeObject(int x, int y) {
		...
		for (Object object : objects) {
			if (object.objectX != x || object.objectY != y) continue;
			if (match != null) return;
			match = object;
		}
		if (match != null) removeObject(match);
		else WorldObjectService.getInstance()
				.applyUniquePendingTimedRemove(this, x, y);
	}

	public void removeObject(int x, int y, int height, int type, int face) {
		WorldObjectService.getInstance().applyTimedRemove(this, x, y, height,
				type, face);
	}

	public void removeObject(Object object) {
		if (object != null) WorldObjectService.getInstance()
				.applyTimedRemove(this, object);
	}

	void projectAuthoritativeRemoval(Object object) {
		for (Player player : PlayerHandler.players) {
			if (player != null) {
				Client c = (Client) player;
				if (loadForPlayer(object, c)) c.getPacketSender().object(-1,
						object.objectX, object.objectY, object.face, object.type);
			}
		}
	}
	...
	public void placeObject(Object o) {
		if (!WorldObjectService.getInstance().isTimedActive(o)) return;
		for (Player player : PlayerHandler.players) {
			if (player != null) {
				Client c = (Client) player;
	...
	public Object getObject(int x, int y, int height, int type, int face) {
		Object match = null;
		int requestedSlot = sceneSlot(type);
		for (Object object : objects) {
			if (object.objectX != x || object.objectY != y || object.height != height
					|| sceneSlot(object.type) != requestedSlot) continue;
			if (object.type != type || object.face != face) continue;
	...
	public void loadObjects(Player c) {
		...
		for (Object o : objects) {
			Objects projection = new Objects(o.objectId, o.objectX, o.objectY,
					o.height, o.face, o.type, 0);
			if (loadForPlayer(o, c) && WorldObjectService.getInstance()
					.shouldProjectLower(c, projection)) {
				c.getPacketSender().object(o.objectId, o.objectX, o.objectY, o.face, o.type);
			}
		}
		loadCustomSpawns(c);
	...
	public void addObject(Object o) {
		Object existing = getObject(o.objectX, o.objectY, o.height, o.type, o.face);
		if (existing == null || existing.objectId != o.objectId) {
			WorldObjectService.getInstance().applyTimedAdd(this, o);
		}
	}

	void applyAuthoritativeAdd(Object object) {
		objects.add(object);
	}

	void applyAuthoritativeRemove(Object object) {
