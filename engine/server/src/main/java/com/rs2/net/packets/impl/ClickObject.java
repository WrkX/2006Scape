...
public class ClickObject implements PacketType {
	...
	public void onObjectReached(Player player, BiConsumer<Player, Objects> consumer) {
		...
		ResolvedWorldObject authoritative = WorldObjectService.getInstance()
				.resolvePacketObject(player, objectId, objectX, objectY,
						player.heightLevel);
		if (authoritative == null) return;
		Objects object = authoritative.getObject();
		if (System.currentTimeMillis() - player.clickDelay < 300) return;
		...
		final Objects resolvedObject = object;
		if (BrimhavenAgilityCourse.canInteractFromDistance(player, objectId, objectX, objectY)) {
			consumer.accept(player, resolvedObject);
			return;
		}

		int[] size = resolvedObject.getObjectSize();
	...
	public void processPacket(final Player player, Packet packet) {
		...
		switch (packet.getOpcode()) {
			...
			case FIRST_CLICK:
				...
				onObjectReached(player, (p, object) -> completeObjectClick(p, 1, object));
				break;
			...
			case SECOND_CLICK:
				...
				onObjectReached(player, (p, object) -> completeObjectClick(p, 2, object));
				break;
			...
			case THIRD_CLICK: // 'F'
				...
				player.objectId = packet.readUnsignedWordBigEndianA();
				onObjectReached(player, (p, object) -> completeObjectClick(p, 3, object));
				break;
			...
			case FOURTH_CLICK:
				...
				onObjectReached(player, (p, object) -> completeObjectClick(p, 4, object));
				break;
			case FIFTH_CLICK:
				player.objectId = packet.readUnsignedWordA();
				player.objectY = packet.readUnsignedWordA();
				player.objectX = packet.readUnsignedWord();
				onObjectReached(player, (p, object) -> completeObjectClick(p, 5, object));
				break;
		}
	}
	...
	public void completeObjectClick(final Player player, int objectOption,
		...
		case 1:
			...
			player.getObjects().firstClickObject(player.objectId, player.objectX,
					player.objectY, clickedObject);
			boolean handledByScript =
					player.getObjects().wasLastClickHandledByScript();
			postLegacyEventIfUnscripted(player, handledByScript,
					new ObjectFirstClickEvent(player.objectId));
			break;
		...
		case 2:
			...
			player.getObjects().secondClickObject(player.objectId, player.objectX,
					player.objectY, clickedObject);
			handledByScript = player.getObjects().wasLastClickHandledByScript();
			postLegacyEventIfUnscripted(player, handledByScript,
					new ObjectSecondClickEvent(player.objectId));
			break;
		...
		case 3: // 'F'
			...
			player.getObjects().thirdClickObject(player.objectId, player.objectX,
					player.objectY, clickedObject);
			handledByScript = player.getObjects().wasLastClickHandledByScript();
			postLegacyEventIfUnscripted(player, handledByScript,
					new ObjectThirdClickEvent(player.objectId));
			break;
		...
		case 4:
			...
			player.getObjects().fourthClickObject(player.objectId, player.objectX,
					player.objectY, clickedObject);
			handledByScript = player.getObjects().wasLastClickHandledByScript();
			postLegacyEventIfUnscripted(player, handledByScript,
					new ObjectFourthClickEvent(player.objectId));
			break;
		}
	}
}
