...
public class Walking implements PacketType {
	...
	public void processPacket(Player player, Packet packet) {
		if (!encounterMovementAllowed(player, packet)) {
			return;
		}
...
//}
	...
	/**
	 * Non-mutating preview of every requested walking waypoint. This runs
	 * before the legacy handler's duel, appearance, teleport, skilling, task,
	 * interface, and queue side effects.
	 */
	private static boolean encounterMovementAllowed(Player player,
			Packet packet) {
		...
		int steps = (packetSize - 5) / 2 + 1;
		if (packetSize < 5 || steps < 1
				|| steps > player.walkingQueueSize
				|| packet.getPayload().readableBytes() < packetSize) {
			return false;
		}
		Packet preview = new Packet(packet.getOpcode(), packet.getType(),
				packet.getPayload().duplicate());
		int firstX;
		int firstY;
		int[] offsetsX = new int[steps];
		int[] offsetsY = new int[steps];
		if (player.clickToTele) {
			firstX = preview.readSignedWordBigEndianA();
		} else {
			firstX = preview.readSignedWordBigEndianA();
		}
		for (int index = 1; index < steps; index++) {
			offsetsX[index] = preview.readSignedByte();
			offsetsY[index] = preview.readSignedByte();
		}
		firstY = preview.readSignedWordBigEndian();
		preview.readSignedByteC();
		ScriptEncounterService encounters =
				ScriptEncounterService.getInstance();
		for (int index = 0; index < steps; index++) {
			int x = firstX + offsetsX[index];
			int y = firstY + offsetsY[index];
			if (!encounters.canDestination(player, x, y,
					player.heightLevel)) {
				return false;
			}
		}
