...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player player, Packet packet) {
		if (!ScriptInteractionGate.hasExactPayload(packet, 2)) {
			return;
		}
		int first = packet.getUnsignedByte();
		int second = packet.getUnsignedByte();
		int actionButtonId = first * 1000 + second;
		if (!ScriptInteractionGate.fullyDecoded(packet)
				|| !ScriptInteractionGate.isCommonPlayerValid(player)) {
			return;
		}
		if (DialogueOptions.handleScriptDialogueOption(player, actionButtonId)) {
			return;
		}
