...
public class ItemClick3 implements PacketType {
	...
	public void processPacket(Player player, Packet packet) {
		...
		int itemSlot = packet.readSignedWordA();
		...
		boolean interactionBlocked =
				player.duelStatus > 0 && player.duelStatus < 5 || player.tradeStatus == 1;
		if (interactionBlocked) {
			return;
		}
		if (ClickItem.executeScriptItemClick(player, itemId, itemSlot, "third")) {
			return;
		}
