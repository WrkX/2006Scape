...
public class Commands implements PacketType {
    ...
    public void processPacket(Player player, Packet packet) {
        String rawInput = packet.readString();
        String trimmedInput = rawInput == null ? "" : rawInput.trim();
        if (trimmedInput.isEmpty()) {
            return;
        }
        String[] messageArr = trimmedInput.split("\\s+");
        ...
        if (executeScriptCommand(player, playerCommand, rawInput, commandArguments)) {
            return;
        }
    ...
    static boolean executeScriptCommand(Player player, String playerCommand) {
        return executeScriptCommand(player, playerCommand, playerCommand, new String[0]);
    }
    ...
    public static void playerCommands(Player player, String playerCommand, String[] arguments) {
        String cmd = playerCommand.toLowerCase(Locale.ROOT);
        switch (cmd) {
            case "stuck":
                long stuckNow = System.currentTimeMillis();
                long stuckCooldown = 60 * 60 * 1000;
                if (player.getLastStuckUse() != 0
                        && stuckNow - player.getLastStuckUse() < stuckCooldown) {
                    long remainingTime = stuckCooldown - (stuckNow - player.getLastStuckUse());
                    long minutesLeft = TimeUnit.MILLISECONDS.toMinutes(remainingTime);
                    long secondsLeft = TimeUnit.MILLISECONDS.toSeconds(remainingTime)
                            - TimeUnit.MINUTES.toSeconds(minutesLeft);
                    player.getPacketSender().sendMessage(
                            "You need to wait " + minutesLeft + " minutes and " + secondsLeft
                                    + " seconds to use ::stuck again.");
                    break;
                }
                if (player.getPlayerAssistant().nudgeUnstuck()) {
                    player.setLastStuckUse(stuckNow);
                    player.getPacketSender().sendMessage("You've been moved to a nearby tile.");
                } else {
                    player.getPacketSender().sendMessage(
                            "Couldn't find a nearby free tile. Try walking in another direction or contact a staff member.");
                }
                break;
            case "home":
    ...
    public static void developerCommands(Player player, String playerCommand, String[] arguments) {
        switch (playerCommand.toLowerCase()) {
            ...
            case "getobject":
                if (arguments.length == 0) {
                    player.getPacketSender().sendMessage("You must specify an object id: ::getobject 1000");
                    return;
                }
                int getObjectId = Integer.parseInt(arguments[0]);
                player.getPacketSender().sendMessage("Object ID: " + getObjectId);
                break;
            case "stack":
                if (arguments.length == 0) {
                    player.getPacketSender().sendMessage("Must specify an item id: ::stack 995 1000");
                    return;
                }
                int stackItemID = Integer.parseInt(arguments[0]);
                int stackItemAmount = arguments.length >= 2 ? Integer.parseInt(arguments[1]) : 1;
                if (stackItemID <= 10000 && stackItemID >= 0) {
                    if (ItemDefinition.lookup(stackItemID).isStackable()) {
                        player.getItemAssistant().addItem(stackItemID, stackItemAmount);
                    } else {
                        // ItemAssistant.addItem internally stores as (id+1);
                        // noted form is (id+1), so we pass the unmodified id
                        // and let addItem's internal +1 produce the noted id.
                        // The noted id must exist in the item definitions.
                        if (ItemDefinition.lookup(stackItemID + 1) != null) {
                            player.getItemAssistant().addItem(stackItemID + 1, stackItemAmount);
                        } else {
                            player.getPacketSender().sendMessage("Item cannot be noted: " + stackItemID);
                        }
                    }
                } else {
                    player.getPacketSender().sendMessage("No such item.");
