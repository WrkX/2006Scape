...
public class PickupItem implements PacketType {
	...
	public void processPacket(final Player player, Packet packet) {
		...
		final int pickupY = packet.readSignedWordBigEndian();
		final int pickupId = packet.readUnsignedWord();
		final int pickupX = packet.readSignedWordBigEndian();
		if (!ScriptInteractionGate.fullyDecoded(packet)
				|| !ScriptInteractionGate.isCommonPlayerValid(player)
				|| !ScriptInteractionGate.validCoordinate(pickupX)
				|| !ScriptInteractionGate.validCoordinate(pickupY)
				|| !ScriptInteractionGate.isDefinitionBackedItem(pickupId)) {
			return;
		}
		final int pickupPlane = player.heightLevel;
		final GroundItemRef pickupRef =
				GameEngine.itemHandler.resolveVisibleGroundItem(
						player, pickupId, pickupX, pickupY, pickupPlane);
		if (pickupRef == null || ScriptInteractionGate.isActionLocked(player)) {
			return;
		}
		player.pItemY = pickupY;
		player.pItemId = pickupId;
		player.pItemX = pickupX;
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (player.goodDistance(player.getX(), player.getY(), pickupX, pickupY, 1) && player.walkingToItem) {
					container.stop();
				}
			}
			...
			public void stop() {
				...
				int amount = GameEngine.itemHandler.consumeGroundItemExact(
						player, pickupRef, true, 1);
				if (amount <= 0) {
					return;
				}
				ScriptLifecycleService.getInstance().onItemPickup(player,
						pickupId, amount, pickupX, pickupY, pickupPlane);
				if (!player.soundDone) {
