...
public class ItemOnObject implements PacketType {
	...
	public void processPacket(Player player, Packet packet) {
		...
		int itemSlot = packet.readSignedWordBigEndian();
		...
		if (!player.goodDistance(objectX, objectY, player.absX, player.absY, distance)) {
			return;
		}
		ResolvedWorldObject resolvedObject = WorldObjectService.getInstance()
		...
		if (executeScriptItemOnObject(player, itemId, itemSlot,
				resolvedObject.getObject())) {
			return;
		}
		...
		return ScriptHost.getInstance().dispatchActive(
				...
				(generation, route) -> ScriptExecutor.executeRoute(
						...
						new ItemOnObjectScriptContext(
								...
								ScriptedItem.byId(itemId), itemSlot,
								new ScriptedObject(object))))
