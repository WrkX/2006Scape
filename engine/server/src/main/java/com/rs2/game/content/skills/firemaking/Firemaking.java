...
public class Firemaking {
	...
	public static void attemptFire(final Player c, final int itemUsed, final int usedWith, final int x, final int y, final boolean groundObject) {
		attemptFire(c, itemUsed, usedWith, x, y, groundObject, null);
	}

	public static void attemptFire(final Player c, final int itemUsed,
			final int usedWith, final int x, final int y,
			final boolean groundObject, final GroundItemRef resolvedGroundItem) {
		if (groundObject && resolvedGroundItem == null) {
			return;
		}
		...
		for (final LogData l : LogData.values()) {
			...
			if (logId == l.getLogId()) {
				...
				c.logLit = false;
					...
					int cycle = 2;
					GroundItemRef fireLog = resolvedGroundItem;
					if (notInstant) {
					...
					if (c.tutorialProgress == 4) {
						...
						if (groundObject == false) {
							...
							fireLog = GameEngine.itemHandler.createGroundItemRef(c,
									logId, c.absX, c.absY, 1, c.playerId);
						}
					...
					final GroundItemRef scheduledLog = fireLog;
				...
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					@Override
						...
						if (c.isWoodcutting || c.playerIsFletching || c.isFletching) {
							...
							if (c.isFiremaking) {
								if (GameEngine.itemHandler.consumeGroundItemExact(c,
										scheduledLog, false, 1) <= 0) {
									container.stop();
									return;
								}
							...
							c.getPlayerAssistant().addSkillXP((int) l.getXp(), 11);
								...
								CycleEventHandler.getSingleton().addEvent(c,
										new CycleEvent() {
									@Override
									public void execute(CycleEventContainer ashContainer) {
										if (c.playerIsCooking) {
											Cooking.setCooking(c, false);
										}
										GameEngine.objectHandler.createAnObject(
												c, -1, x, y);
