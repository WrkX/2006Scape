...
public class PlayerHandler {
	...
	public void updateNPC(Player plr, Stream str) {
		...
		for (int i = 0; i < size; i++) {
			if (plr.rebuildNPCList == false && plr.withinDistance(plr.npcList[i])
					&& ScriptNpcService.getInstance().canAct(plr.npcList[i], plr)) {
		...
		for (Npc i : NpcHandler.npcs) {
			if (i != null) {
				if (!ScriptNpcService.getInstance().canAct(i, plr)) {
					continue;
				}
				int id = i.npcId;
	...
	void initializePlayer(Player player) {
		...
		ScriptLifecycleService.getInstance().onLogin(player);
		BrimhavenAgilityArena.onPlayerInitialized(player);
	}
	...
	void removePlayer(Player plr) {
		...
		ScriptLifecycleService.getInstance().onPlayerRemoved(plr);
		plr.destruct();
	}
