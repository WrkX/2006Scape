...
public class DialogueOptions {
	...
	public static void handleDialogueOptions(Player player, int buttonId) {
		...
		case 9158:
			if (player.dialogueAction == 8) {
				...
				player.getPacketSender().showInterface(3559);
				player.canChangeAppearance = true;
				player.tutorialMakeoverPending = true;
				player.getPacketSender().closeAllWindows();
				return;
			} else if (player.dialogueAction == 154) {
	...
	static int decodeOptionChoice(int buttonId, int optionCount) {
		switch (optionCount) {
			case 2:
				return buttonId == 9157 ? 0 : buttonId == 9158 ? 1 : -1;
			case 3:
				return buttonId >= 9167 && buttonId <= 9169 ? buttonId - 9167 : -1;
			case 4:
				return buttonId >= 9178 && buttonId <= 9181 ? buttonId - 9178 : -1;
			case 5:
				return buttonId >= 9190 && buttonId <= 9194 ? buttonId - 9190 : -1;
			default:
				return -1;
		}
	}
