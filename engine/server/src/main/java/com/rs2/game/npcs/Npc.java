...
public class Npc {
    ...
    private static long nextAllocationToken = 1L;

    /**
     * Monotonic identity for this allocation.  The slot number is reused by
     * the legacy engine, so script continuations must carry this token too.
     */
    private final long allocationToken;
    private final ArrayDeque<ScriptRouteStep> scriptRoute =
            new ArrayDeque<ScriptRouteStep>();
    ...
    public int attackType, projectileId, endGfx, spawnedBy, hitDelayTimer, HP,
            MaxHP, hitDiff, animNumber, animDelay, actionTimer, enemyX, enemyY,
            combatLevel;
    ...
    public Npc(int _npcId, int _npcType) {
        synchronized (Npc.class) {
            allocationToken = nextAllocationToken++;
        }
        npcId = _npcId;
        npcType = _npcType;
        direction = -1;
    ...
    /** Queues one authoritative script movement step for the next NPC tick. */
    public synchronized boolean queueScriptRouteStep(int x, int y, int plane) {
        if (plane != heightLevel || !scriptRoute.isEmpty()
                || x < 0 || y < 0 || x > 16383 || y > 16383
                || Math.abs(x - absX) > 1 || Math.abs(y - absY) > 1
                || (x == absX && y == absY)
                || !PathFinder.isStepClear(absX, absY, heightLevel, x, y)) {
            return false;
        }
        scriptRoute.addLast(new ScriptRouteStep(x, y, plane));
        return true;
    }

    /** Applies one previously queued script step from the NPC engine tick. */
    public synchronized boolean processScriptRouteStep() {
        ScriptRouteStep next = scriptRoute.pollFirst();
        if (next == null || next.plane != heightLevel
                || !PathFinder.isStepClear(absX, absY, heightLevel,
                        next.x, next.y)) {
            return false;
        }
        moveX = next.x - absX;
        ...
        absX = next.x;
        absY = next.y;
        direction = Misc.direction(absX - moveX, absY - moveY, absX, absY) >> 1;
        updateRequired = true;
        return true;
    }

    private static final class ScriptRouteStep {
        final int x;
        final int y;
        final int plane;

        ScriptRouteStep(int x, int y, int plane) {
            this.x = x;
            this.y = y;
            this.plane = plane;
        }
