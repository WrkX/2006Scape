...
import com.rs2.script.state.ScriptStateException;
...
public class PlayerSave {
	...
	public static int loadPlayerInfo(Client player, String playerName, String playerPass, boolean doRealLogin)	{
		...
		boolean File1 = false;
		String scriptStatePayload = null;
		boolean duplicateScriptStatePayload = false;
			...
			if (spot > -1) {
					...
					case 2:
						switch (token) {
							...
							case "cookAss":
								...
								break;
							case "cookStage1":
								player.cookStage1 = Integer.parseInt(token2);
								break;
							case "bananas":
							...
							case "questPoints":
								...
								break;
							case "character-script-state":
								if (scriptStatePayload != null) {
									duplicateScriptStatePayload = true;
								} else {
									scriptStatePayload = token2;
								}
								break;
							case "votePoints":
				...
				switch (line) {
					...
					case "[NPC-KILLS]":
						...
						break;
					case "[EOF]":
						installScriptState(player, scriptStatePayload,
								duplicateScriptStatePayload);
						try {
							characterfile.close();
	...
	public static boolean saveGame(Player player) {
		...
		if (player.getQuarantinedScriptStatePayload() != null) {
			System.err.println(player.playerName
					+ ": refusing to save while malformed script state remains "
					+ "quarantined");
			return false;
		}
		final String scriptStatePayload;
		try {
			scriptStatePayload = new ScriptStateCodec().encode(
					player.getScriptState().snapshot());
		} catch (ScriptStateException e) {
			System.err.println(player.playerName
					+ ": refusing to save invalid script state: " + e.getMessage());
			return false;
		}
		...
		try {
			...
			temporaryFile = File.createTempFile(player.playerName + ".", ".tmp",
					targetFile.getParentFile());
			characterfile = new BufferedWriter(new OutputStreamWriter(
			...
			characterfile.newLine();
			characterfile.write("character-script-state = " + scriptStatePayload);
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("cookStage1 = " + player.cookStage1);
			characterfile.newLine();
	...
	static void installScriptState(Player player, String payload,
			boolean duplicate) {
		ScriptStateSnapshot candidate;
		if (payload == null && !duplicate) {
			candidate = new com.rs2.script.state.ScriptStateStore().snapshot();
			player.setQuarantinedScriptStatePayload(null);
		} else {
			try {
				if (duplicate) {
					throw new ScriptStateException(
							"Duplicate character-script-state field");
				}
				candidate = new ScriptStateCodec().decode(payload);
				player.setQuarantinedScriptStatePayload(null);
			} catch (ScriptStateException e) {
				player.setQuarantinedScriptStatePayload(payload);
				System.err.println(player.playerName
						+ ": quarantined script state ("
						+ (payload == null ? 0 : payload.length()) + " chars): "
						+ e.getMessage());
				candidate = new com.rs2.script.state.ScriptStateStore().snapshot();
			}
		}
		player.getScriptState().replace(candidate);
	}
