...
public class CombatAssistant {
	...
	public void delayedHit(int i) { // npc hit delay
		if (i < 0 || i >= NpcHandler.npcs.length
				...
				|| (player.oldNpcAllocationToken != 0L
						&& player.oldNpcAllocationToken != NpcHandler.npcs[i].allocationToken())
				|| !ScriptNpcService.getInstance().canAct(NpcHandler.npcs[i], player)) {
	...
	public void fireProjectileNpc() {
		if (player.oldNpcIndex > 0) {
			if (NpcHandler.npcs[player.oldNpcIndex] != null
					&& (player.oldNpcAllocationToken == 0L
							|| player.oldNpcAllocationToken == NpcHandler.npcs[player.oldNpcIndex].allocationToken())
					&& ScriptNpcService.getInstance().canAct(
							NpcHandler.npcs[player.oldNpcIndex], player)) {
	...
	public void attackingNpcTick() {
		...
		if (i > 0 && (i >= NpcHandler.npcs.length
				...
				|| (player.npcAllocationToken != 0L
						&& player.npcAllocationToken != NpcHandler.npcs[i].allocationToken())
				|| !ScriptNpcService.getInstance().canAct(NpcHandler.npcs[i], player))) {
			clearNpcContinuation();
			return;
		}
	...
	public void attackNpc(int i) {
		...
		player.npcIndex = requestedNpcIndex;
		player.npcAllocationToken = NpcHandler.npcs[i].allocationToken();
		if (NpcHandler.npcs[i] != null) {
	...
	/** Clears every continuation field that can target a reused NPC slot. */
	public void clearNpcContinuation() {
		player.npcIndex = 0;
		player.npcAllocationToken = 0L;
		player.oldNpcIndex = 0;
		player.oldNpcAllocationToken = 0L;
