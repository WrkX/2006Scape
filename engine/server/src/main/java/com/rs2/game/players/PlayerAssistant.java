...
public class PlayerAssistant {
	...
	/**
	 * Nudge a stuck player one tile in a random direction when collision allows it.
	 */
	public boolean nudgeUnstuck() {
		int x = player.absX;
		int y = player.absY;
		...
		for (int dir : directions) {
			int destX = x + Misc.directionDeltaX[dir];
			int destY = y + Misc.directionDeltaY[dir];
			if (!isStandableTile(destX, destY, h)) {
				continue;
			}
			if (!Region.canMove(x, y, destX, destY, h, 1, 1)) {
				continue;
			}
			movePlayer(destX, destY, h);
			return true;
		}
		return false;
	}

	private static boolean isStandableTile(int x, int y, int h) {
		int clip = Region.getClipping(x, y, h);
		return (clip & 0x20000) == 0 && (clip & 0x100) == 0;
	}
	...
	public boolean addSkillXP(double amount, int skill) {
		if (skill < 0 || skill >= player.playerXP.length
				|| Double.isNaN(amount) || Double.isInfinite(amount) || amount <= 0
				|| player.playerXP[skill] < 0 || player.playerXP[skill] > 200000000) {
			return false;
		}
		...
		double resultingExperience = player.playerXP[skill] + amount;
		if (Double.isNaN(resultingExperience) || Double.isInfinite(resultingExperience)
				|| resultingExperience > 200000000 || resultingExperience < 0) {
			return false;
		}
		int nextExperience = (int) resultingExperience;
		if (nextExperience <= player.playerXP[skill]) {
			return false;
		}
		int oldLevel = getLevelForXP(player.playerXP[skill]);
		player.playerXP[skill] = nextExperience;
