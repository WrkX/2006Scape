...
public class NpcHandler {
    ...
    /** Returns a valid backing player, or null for an unowned/null killer. */
    private static Player playerAt(int playerId) {
        if (playerId < 0 || playerId >= PlayerHandler.players.length) {
            return null;
        }
        return PlayerHandler.players[playerId];
    }
    ...
    public static boolean hasNpcDefinition(int npcType) {
        ...
        for (int i = 0; i < maxListedNPCs; i++) {
            NpcList definition = NpcList[i];
            if (definition != null && definition.npcId == npcType
                    && definition.npcName != null
                    && !definition.npcName.trim().isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    public int getClosePlayer(Player c, int i) {
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
                if (!ScriptNpcService.getInstance().canAct(npcs[i],
                        PlayerHandler.players[j])) {
                    continue;
                }
    ...
    private boolean killedBarrow(int i) {
        ...
        Player c = playerAt(npcs[i].killedBy);
        if (c != null) {
    ...
    public void process() {
        ...
        for (int i = 0; i < MAX_NPCS; i++) {
            if (npcs[i] != null) {
				boolean scriptedRouteStep = npcs[i].processScriptRouteStep();
		    ...
		    if (npcs[i].npcType == GRAVINGAS)
		    {
				...
				Player client = closePlayerId > 0
						? PlayerHandler.players[closePlayerId] : null;
				if (client != null && !ScriptNpcService.getInstance().canAct(
						npcs[i], client)) {
					client = null;
				}
				if (client != null) {
                ...
                if ((npcs[i].killerId > 0 || npcs[i].underAttack) && !npcs[i].walkingHome && retaliates(npcs[i].npcType)) {
                    if (!npcs[i].isDead) {
						...
						if (p >= 0 && p < PlayerHandler.players.length
								&& PlayerHandler.players[p] != null
								&& ScriptNpcService.getInstance().canAct(npcs[i],
										PlayerHandler.players[p])) {
                ...
                if (npcs[i].isDead) {
                    if (npcs[i].actionTimer == 0 && npcs[i].applyDead == false && npcs[i].needRespawn == false) {
                        ...
                        Npc deathNpc = npcs[i];
                        if (!ScriptNpcService.getInstance().beginDeath(deathNpc)) {
                            continue;
                        }
                        ScriptedPosition deathPosition = new ScriptedPosition(
                                deathNpc.absX, deathNpc.absY, deathNpc.heightLevel);
                        ScriptNpcSnapshot deathSnapshot =
                                ScriptNpcSnapshot.capture(deathNpc);
                        int deathHp = deathNpc.HP;
                        boolean deathWasDead = deathNpc.isDead;
                        int deathCombatLevel = deathNpc.combatLevel;
                        for (Player continuation : PlayerHandler.players) {
                            if (continuation != null
                                    && ((continuation.npcAllocationToken != 0L
                                            && continuation.npcAllocationToken == deathNpc.allocationToken())
                                    || (continuation.oldNpcAllocationToken != 0L
                                            && continuation.oldNpcAllocationToken == deathNpc.allocationToken())
                                    || continuation.npcIndex == i
                                    || continuation.oldNpcIndex == i
                                    || continuation.followNpcId == i)) {
                                continuation.getCombatAssistant()
                                        .clearNpcContinuation();
                            }
                        }
                        boolean ownedDeath = ScriptNpcService.getInstance()
                                .isExactOwned(deathNpc);
                        npcs[i].needRespawn = true;
                        npcs[i].actionTimer = NpcData.getRespawnTime(i); // respawn
                        // time; an encounter-owned NPC supplies its own drops.
                        if (!ownedDeath) {
                            dropItems(i); // npc drops items!
                        }
                        if (playerAt(npcs[i].spawnedBy) != null) {
                        ...
                        if (npcs[i].npcType == TZTOKJAD) {
                            ...
                            Player killer = playerAt(npcs[i].killerId);
                            if (killer != null) {
                                killer.spawnedHealers = 0;
                                killer.canHealersRespawn = true;
                            }
                        }
                        if (npcs[i].npcType == FightCaves.YT_HURKOT) {
                            Player killer = playerAt(npcs[i].killerId);
                            if (killer != null) {
                                if (killer.spawnedHealers != 0) {
                                    killer.spawnedHealers -= 1;
                                    if (killer.spawnedHealers <= 0) {
                                        killer.spawnedHealers = 0;
                                    }
                                }
                        ...
                        resetEvent(i);
                        Player player = playerAt(npcs[i].killedBy);
                        if (player != null) {
                        ...
                        if (player != null && npcs[i].npcType > RABBIT_3726
                                && npcs[i].npcType < SHIFTER) {
                        ...
                        if (player != null && npcs[i].npcType == TREE_SPIRIT_655) {
                            player.spiritTree = true;
                            player.getPacketSender().sendMessage(
                                    "You have defeated the tree spirit.");
                        }
                        if (player != null && npcs[i].npcType > BAT
                                && npcs[i].npcType < ZOMBIE_419) {
                            player.golemSpawned = false;
                        }
                        if (player != null && npcs[i].npcType == COUNT_DRAYNOR
                                && player.vampSlayer == 3) {
                            player.vampSlayer = 4;
                        }
                        ...
                        if (player != null && npcs[i].npcType > ROCK_GOLEM_418
                                && npcs[i].npcType < SHADE) {
                            player.zombieSpawned = false;
                        }
                        if (player != null && npcs[i].npcType > ZOMBIE_424
                                && npcs[i].npcType < WATCHMAN_431) {
                            player.shadeSpawned = false;
                        }
                        if (player != null && npcs[i].npcType > CAPN_IZZY_NOBEARD
                                && npcs[i].npcType < GOBLIN_444) {
                            player.treeSpiritSpawned = false;
                        }
                        if (player != null && npcs[i].npcType > PHEASANT_2462
                                && npcs[i].npcType < FROG_2469) {
                            player.chickenSpawned = false;
                        }
                        Player lifecycleKiller = playerAt(deathNpc.killedBy);

                        // Complete all legacy state and slot/object dereferences
                        // before guest callbacks.  Callbacks receive only the
                        // captured immutable identity below.
                        int deathType = deathNpc.npcType;
                        int deathX = deathNpc.absX;
                        int deathY = deathNpc.absY;
                        deathNpc.absX = deathNpc.makeX;
                        deathNpc.absY = deathNpc.makeY;
                        deathNpc.HP = deathNpc.MaxHP;
                        deathNpc.animNumber = 0x328;
                        deathNpc.updateRequired = true;
                        deathNpc.animUpdateRequired = true;
                        if (deathType >= DOORSUPPORT && deathType <= DOORSUPPORT_2446) {
                            GameEngine.objectManager.removeObject(deathX, deathY);
                        }
                        ScriptLifecycleService.getInstance().onNpcDeath(
                                deathNpc, lifecycleKiller, deathSnapshot,
                                deathPosition, deathHp, deathWasDead,
                                deathCombatLevel);
                        ScriptNpcService.getInstance().dispatchDeath(deathNpc,
                                lifecycleKiller, deathPosition, deathSnapshot);
                        ScriptNpcService.getInstance().finishDeath(deathNpc);
    ...
    private void handleratdeath(int i) {
        final Player c = playerAt(npcs[i].killedBy);
        if (c != null) {
    ...
    public static void followPlayer(int i, Player player) {
        if (i < 0 || i >= npcs.length || npcs[i] == null || player == null
                || !ScriptNpcService.getInstance().canAct(npcs[i], player)) {
            return;
        }
