...
public class NpcCombat {
	...
	public static void multiAttackDamage(int i) {
		...
		for (Player player : PlayerHandler.players) {
			if (player != null) {
				...
				if (!ScriptNpcService.getInstance().canAct(
						NpcHandler.npcs[i], c)) {
					continue;
				}
	...
	public static void attackPlayer(Player c, int i) {
		if (i < 0 || i >= NpcHandler.npcs.length || NpcHandler.npcs[i] == null
			...
			if (NpcData.checkClip(NpcHandler.npcs[i]) || special) {
				if (c.respawnTimer <= 0) {
					...
					if (NpcHandler.multiAttacks(i)) {
						...
						NpcHandler.npcs[i].oldAllocationToken = NpcHandler.npcs[i].allocationToken();
						return;
					}
					if (NpcHandler.npcs[i].projectileId > 0) {
						int nX = NpcHandler.npcs[i].getX() + NpcHandler.offset(i);
	...
	public static void registerNpcHit(int i) {
		...
		if (NpcHandler.npcs[i].oldAllocationToken != 0L
				&& NpcHandler.npcs[i].oldAllocationToken != NpcHandler.npcs[i].allocationToken()) {
			clearNpcContinuation(NpcHandler.npcs[i]);
			return;
		}
		int oldIndex = NpcHandler.npcs[i].oldIndex;
		if (oldIndex < 0 || oldIndex >= PlayerHandler.players.length
				|| PlayerHandler.players[oldIndex] == null
				|| NpcHandler.npcs[i].isDead) {
			clearNpcContinuation(NpcHandler.npcs[i]);
			return;
		}
		Client c = (Client) PlayerHandler.players[oldIndex];
		if (!ScriptNpcService.getInstance().canAct(NpcHandler.npcs[i], c)) {
			clearNpcContinuation(NpcHandler.npcs[i]);
			return;
		}
			if (NpcHandler.multiAttacks(i)) {
	...
	private static void clearNpcContinuation(com.rs2.game.npcs.Npc npc) {
		npc.oldIndex = 0;
		npc.oldAllocationToken = 0L;
	}
