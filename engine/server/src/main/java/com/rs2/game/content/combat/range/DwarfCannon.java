...
public class DwarfCannon {
	...
	public void placeCannon() {
		...
		public void shootNpcs() {
			...
			if (target != null) {
				if (!ScriptNpcService.getInstance().canAct(target, player)) {
					return;
				}
				if (damage > target.HP) {
					damage = target.HP;
		...
		private Npc targetNpc() {
			for (int i = 0; i < NpcHandler.MAX_NPCS; i++) {
				...
				if (npc == null || npc.heightLevel != player.heightLevel
						|| !ScriptNpcService.getInstance().canAct(npc, player)
						|| !canAttackSlayer(i)) {
					continue;
				}
		...
		private void cannonProjectile(Npc n) {
			if (n == null || !ScriptNpcService.getInstance().canAct(n, player)) {
				return;
			}
