...
//		Preconditions.checkElementIndex(id, definitions.length, "Id out of bounds.");
	...
	/**
	 * Returns whether an id is backed by a real cache definition.  A number of
	 * legacy callers intentionally use {@link #lookup(int)}'s NULL_DEF sentinel;
	 * script-facing APIs must not treat that sentinel as a valid item.
	 */
	public static boolean exists(int id) {
		return definitions != null && id >= 0 && id < definitions.length
				&& definitions[id] != null && definitions[id].getId() == id;
	}
